...
export class AuthResolver extends Database {

  public async me(_args: any, context: any): Promise<Person | null> {
  ...
  public async login({ password, name }: { password: string, name: string }, context: any): Promise<Person | null> {
  ...
  public async logout({ _id }: { _id: string }, context: any): Promise<boolean> {
    const res: Response = (await context).response;
    const req: Request = (await context).request;
    const collection = (await Database.db).collection("persons");
  ...
  public async changePw({ _id, newPassword }: { _id: string, newPassword: string }, context: any): Promise<boolean> {
    const req: Request = (await context).request;
