...
export class EventMultiEffects {
  ...
  @Effect()
  checkForRespondableEvents$ = this.actions$.pipe(
    ...
    map(({ respondable, user }) => new UpdateRespondableEventAmount({ respondableEvents: respondable }))
  );

  @Effect()
  loadEventsForMonth$ = this.actions$.pipe(
